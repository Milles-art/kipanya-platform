...
final class PublicContentController extends Controller
{
    ...
    public function home(): View
    {
        ...
        $featured = $this->hasArtwork($this->published()->with('category'))->where('is_featured', true)->latest('published_at')->take(6)->get();
        $latest = $this->hasArtwork($this->published()->with('category'))->latest('published_at')->take(12)->get();
        ...
        $collections = Collection::query()->where('is_active', true)->with(['cartoons' => fn ($q) => $this->hasArtwork($q->where('status', ContentStatus::Published->value))->with('category')->orderBy('cartoon_collection.sort_order')->take(6)])->orderBy('sort_order')->orderBy('name')->take(3)->get();
        return view('pages.public.home', compact('featured', 'latest', 'categories', 'collections'));
    }

    public function cartoon(): View
    {
        $featured = $this->hasArtwork($this->published()->with('category')->where('is_featured', true))->latest('published_at')->first();
        $daily = $this->hasArtwork($this->published()->with('category'))
            ->where('is_daily', true)
            ...
            ->take(10)->get();
        $latest = $this->hasArtwork($this->published()->with('category'))->latest('published_at')->latest('id')->take(12)->get();
        ...
        $collections = Collection::query()->where('is_active', true)
            ->with(['cartoons' => fn ($q) => $this->hasArtwork($q->where('status', ContentStatus::Published->value))->with('category')->orderBy('cartoon_collection.sort_order')->take(6)])
            ->withCount(['cartoons' => fn ($q) => $this->hasArtwork($q->where('status', ContentStatus::Published->value))])
    ...
    {
        $query = $this->hasArtwork($this->published()->with('category'))
    ...
    public function category(Category $category): View
    {
        ...
        $cartoons = $this->hasArtwork($this->published()->with('category'))
    ...
    public function collection(Collection $collection): View
    {
        ...
        $collection->load(['cartoons' => fn ($q) => $this->hasArtwork($q->where('status', ContentStatus::Published->value))->with('category')->orderBy('cartoon_collection.sort_order')->orderByDesc('published_at')]);
    ...
    public function detail(Cartoon $cartoon): View
    {
        ...
        $related = $this->hasArtwork($this->published()->with('category'))
